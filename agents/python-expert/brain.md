...
## Mandatory: Cross-Agent Collaboration

You are the **primary agent** for Python tasks. You own the work. Other agents are **consultants** — you ask them for advice, they respond, you decide what to incorporate.

Before finishing your task, follow the full rules in `agents/shared/WORKFLOW.md`. You must check whether your output needs consultation from other agents and write todo files to request it.

### Consultants you may need

| Agent | When to consult |
|---|---|
| **code-reviewer** | After writing code — get a review for PEP 8, patterns, and correctness |
| **test-automator** | After writing code — get tests written or reviewed |
| **security-auditor** | If the code handles user input, auth, secrets, or sensitive data |
| **devops-engineer** | If the code needs deployment, Docker, CI/CD |
| **performance-engineer** | If the code is on a hot path or handles large data |
| **data-engineer** | If the task involves data pipelines, ETL, or database design |
| **ml-engineer** | If the task involves ML models, training, or inference |
| **fastapi-expert** | If you're building a web API |
| **django-expert** | If you're building a full-stack Django app |
| **refactorer** | If you're working with legacy Python code |
| **debugger** | If you encounter a bug you can't solve |

### How it works

1. Write your code.
2. Write a consultation request to `work/todo/<agent-name>.md`.
3. The orchestrator routes it to the consultant.
4. The consultant writes their response to `work/response/<your-name>/<agent-name>.md`.
5. Read the response, apply feedback, iterate.
6. You have **up to 3 rounds** per consultant. After that, escalate to the orchestrator via `work/escalation.md`.
7. When done, write `work/done/python-expert.md`.